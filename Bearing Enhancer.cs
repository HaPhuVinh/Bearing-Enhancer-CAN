...
namespace Bearing_Enhancer_CAN
{
    ...
    public class Bearing_Enhancer
    {
        ...
        public string DurationFactor { get; set; }
        ...
        public string BBlock_Label = "BB1";
        ...
        public List<VerticalWebCandidate> Check_VerticalWeb_Candiadate(Top_Plate_Info topplate, List<(int No_, string Name, string key, string[] Cordinates_LeftEnd, string[] Cordinates_RightEnd)> lumberpieces, List<LumberInventory> listlumberinventory, (string[] leftcordinates, string[] rightcordinates) chordcoordinates)
        {
            ...
            if (topplate.YLocation == "BottomChord" || topplate.YLocation == "BotChd"|| topplate.YLocation == "Web" || topplate.YLocation == "BrgBlock")
            {
                ...
                for (int i = 0; i < chordcoordinates.leftcordinates.Length; i += 3)
                {
                ...
                string[] chord_LeftPoint_Top = Left_Chord_Coordinates[Left_Chord_Coordinates.Count-1];
                
                string[] chord_RightPoint_Top = Right_Chord_Coordinates[0];
                string[] chord_RightPoint_Bot = Right_Chord_Coordinates[Right_Chord_Coordinates.Count-1];
    ...
    public class Bearing_Enhancer_VerBlock : Bearing_Enhancer
    {
        ...
        public override string Generate_Enhancer_Note(string chosensolution, string language, string unit, VerticalWebCandidate web)
        {
            ...
            if (language == "English")
            {
                ...
                {
                    theNote = $"Attach bearing block {BBlock_Label}, {lumSize}x{Hor_Block.BlockLength}{iom.Text} {LumSpecie} #2 (or better), to {(Hor_Block.NumberBlock == 1 ? "one face" : "both faces")} of the web {web.IDName} w/ {(row)} {(row > 1 ? "staggered rows" : "row")} of {fasDescription} @ {Hor_Block.MinSpacing}{iom.Text} o.c. {(row > 1 ? "Stagger rows by 1/2 the nails spacing." : ".")} Install a minimum of ({(Hor_Block.NumberBlock == 2 ? (Math.Ceiling(numberFastener * 1.0 / 2)) : numberFastener)}) nails{(Hor_Block.NumberBlock == 2 ? " per block." : ".")}";
                }
                else if (fastenerType.Contains("SDW"))
                {
                    theNote = $"Attach bearing block {BBlock_Label}, {lumSize}x{Hor_Block.BlockLength}{iom.Text} {LumSpecie} #2 (or better), to {(Hor_Block.NumberBlock == 1 ? "one face" : "both faces")} of the web {web.IDName} w/ {(row)} {(row > 1 ? "staggered rows" : "row")} of Simpson {fasDescription} @ {Hor_Block.MinSpacing}{iom.Text} o.c. Install the screws per Simpson specifications. Install a minimum of ({(Hor_Block.NumberBlock == 2 ? (Math.Ceiling(numberFastener * 1.0 / 2)) : numberFastener)}) screws{(Hor_Block.NumberBlock == 2 ? " per block." : ".")}";
                }
                else if (fastenerType.Contains("SDS"))
                {
                    theNote = $"Attach bearing block {BBlock_Label}, {lumSize}x{Hor_Block.BlockLength}{iom.Text} {LumSpecie} #2 (or better), to {(Hor_Block.NumberBlock == 1 ? "one face" : "both faces")} of the web {web.IDName} w/ ({(Hor_Block.NumberBlock == 2 ? (Math.Ceiling(numberFastener * 1.0 / 2)) : numberFastener)}) Simpson {fasDescription}{(Hor_Block.NumberBlock == 2 ? " per block." : ".")} See ICC-ES Report ESR-2236 for minimum spacing, edge distance, and end distance requirements for SDS screws.";
                }
                else { theNote = ""; }
            }
            else
    ...
    public class Bearing_Enhancer_HorBlock : Bearing_Enhancer
    {
        ...
        public override string Generate_Enhancer_Note(string chosensolution, string language, string unit, VerticalWebCandidate web = null)
        {
            ...
            if (language == "English")
            {
                if (fastenerType.Contains("Nail"))
                {
                    theNote = $"Attach bearing block {BBlock_Label}, {lumSize}x{Hor_Block.BlockLength}{iom.Text} {LumSpecie} #2 (or better), to {(Hor_Block.NumberBlock == 1 ? "one face" : "both faces")} of the bottom chord w/ {(row)} {(row>1 ? "staggered rows" : "row")} of {fasDescription} @ {Math.Round(Hor_Block.MinSpacing*iom.miliFactor)}{iom.Text} o.c. {(row>1? "Stagger rows by 1/2 the nails spacing." : ".")} Install a minimum of ({(Hor_Block.NumberBlock == 2 ? (Math.Ceiling(numberFastener * 1.0 / 2)) : numberFastener)}) nails{(Hor_Block.NumberBlock == 2 ? " per block." : ".")}";
                }
                else if (fastenerType.Contains("SDW"))
                {
                    theNote = $"Attach bearing block {BBlock_Label}, {lumSize}x{Hor_Block.BlockLength}{iom.Text} {LumSpecie} #2 (or better), to {(Hor_Block.NumberBlock == 1 ? "one face" : "both faces")} of the bottom chord w/ {(row)} {(row > 1 ? "staggered rows" : "row")} of Simpson {fasDescription} @ {Math.Round(Hor_Block.MinSpacing * iom.miliFactor)}{iom.Text} o.c. Install the screws per Simpson specifications. Install a minimum of ({(Hor_Block.NumberBlock == 2 ? (Math.Ceiling(numberFastener * 1.0 / 2)) : numberFastener)}) screws{(Hor_Block.NumberBlock == 2 ? " per block." : ".")}";
                }
                else if (fastenerType.Contains("SDS"))
                {
                    theNote = $"Attach bearing block {BBlock_Label}, {lumSize}x{Hor_Block.BlockLength}{iom.Text} {LumSpecie} #2 (or better), to {(Hor_Block.NumberBlock == 1 ? "one face" : "both faces")} of the bottom chord w/ ({(Hor_Block.NumberBlock == 2 ? (Math.Ceiling(numberFastener * 1.0 / 2)) : numberFastener)}) Simpson {fasDescription}{(Hor_Block.NumberBlock == 2 ? " per block." : ".")} See ICC-ES Report ESR-2236 for minimum spacing, edge distance, and end distance requirements for SDS screws.";
                }
                else { theNote = ""; }
            }
            else
                        ...
                        if (VerticalWeb.Count > 1)
                        {
                            if (leftcordinates.Any(lc => IsPointOnLine(lc, TwoPoint_LineEquation(VerticalWeb[0][0], VerticalWeb[1][VerticalWeb[1].Count - 1]))))//Check vertical web pass through the heel the bottom chord
                            {
                                ...
                                if (slopeBaseLine == 0)
                                {
                                    ...
                                    {
                                        basePoint = VerticalWeb[0][VerticalWeb[0].Count - 1];
                ...
                else if (double.Parse(refPoint[0]) >= double.Parse(basePoint[0]) && (bTopChordOnBottomChord || !bTopChordAboveBottomChord))// && bTopChordOnBottomChord)//check for girder heels
                {
                    ...
                    else //Normal heels
                    {
                        ...
                        if (blocklength <= girderHeelLength)
                        {
                            ...
                            string[] newEndPoint = Intersection_Point(refLineTop, vertical_Line_LeftEnd);

                            Cordinates.Add(newEndPoint);
                        }
                        else//(blocklength > girderHeelLength)
                        {
                            baseLineTop = refLineBot;
                            for (int i = 0; i < leftcordinates.Count; i++)
                            {
            ...
            else if (bearingtype == "Exterior_Right")
            {
                ...
                if (bTopChordAboveBottomChord) // Math.Abs(refLineBot.A * refLineTop.B - refLineBot.B * refLineTop.A) <= 0.01)) check raised heels
                {
                    ...
                    foreach (var web in Webs)
                    {
                        ...
                        if (VerticalWeb.Count > 1)
                        {
                            if (rightcordinates.Any(rc => IsPointOnLine(rc, TwoPoint_LineEquation(VerticalWeb[0][VerticalWeb[0].Count-1], VerticalWeb[1][0]))))//Check vertical web pass through the heel the bottom chord
                            {
                                Cordinates.Clear();
                                if (slopeBaseLine == 0)
                                {
                                    ...
                                    else if (VerticalWeb[0].Any(wc => IsPointBelowLine(wc, baseLineTop)))
                                    {
                                        ...
                                        string[] newPoint1 = Intersection_Point(TwoPoint_LineEquation(VerticalWeb[0][0], VerticalWeb[1][VerticalWeb[1].Count - 1]), baseLineBot);
                                        string[] newPoint2 = Intersection_Point(TwoPoint_LineEquation(VerticalWeb[0][0], VerticalWeb[1][VerticalWeb[1].Count - 1]), baseLineTop);
                                        Cordinates.Add(newPoint1);
                                        Cordinates.Add(newPoint2);
                                    }
                                    else
                                ...
                                {
                                    string[] sRefPoint = Intersection_Point(baseLineTop, TwoPoint_LineEquation(VerticalWeb[0][0], VerticalWeb[1][VerticalWeb[1].Count-1]));
                ...
                else if (double.Parse(refPoint[0]) <= double.Parse(basePoint[0]) && (bTopChordOnBottomChord || !bTopChordAboveBottomChord))// && bTopChordOnBottomChord)//check for girder heels
                {
                    ...
                    else if (double.Parse(refPoint[0]) == double.Parse(rightcordinates[0][0]))//girder heel
                    {
                        ...
                        if (blocklength <= girderHeelLength)
                        {
                            baseLineTop = TwoPoint_LineEquation(rightcordinates[0], rightcordinates[1]);
                            for (int i = rightcordinates.Count - 1; i >= 0; i--)
                            {
                                Cordinates.Add(rightcordinates[i]);
                            }
                    ...
                    else //Normal heels
                    {
                        ...
                        if (blocklength <= girderHeelLength)
                        {
                            ...
                            for (int i = rightcordinates.Count -1 ; i >= 0 ; i--)
                            {
                                if (double.Parse(rightcordinates[i][0]) <= double.Parse(basePoint[0]))
                                {
                                    Cordinates.Add(rightcordinates[i]);
                                }
                                if (Math.Abs(double.Parse(basePoint[0]) - double.Parse(rightcordinates[i][0])) <= tolerance)
                                {
                                    break;
                                }

                            }
                            string[] newEndPoint = Intersection_Point(refLineTop, vertical_Line_RightEnd);
                ...
                else//check for normal heels 
                {
                    ...
                    string[] newEndPoint = Intersection_Point(refLineBot, vertical_Line_RightEnd);
